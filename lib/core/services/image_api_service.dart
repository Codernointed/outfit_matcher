...
class ImageApiService {
  ...
  static Future<List<OnlineInspiration>> searchFashionImages({
    ...
    try {
      ...
      AppLogger.info(
        '✅ Pexels search complete',
        data: {'results': pexelsResults.length},
      );
      results.addAll(pexelsResults);

      AppLogger.debug('🔄 Attempting Unsplash search...');
      final unsplashResults = await _searchUnsplash(
        query,
        limit - pexelsResults.length,
      );
      AppLogger.info(
        '✅ Unsplash search complete',
        data: {'results': unsplashResults.length},
      );
      results.addAll(unsplashResults);
    } catch (e, stackTrace) {
      AppLogger.error(
        '❌ Error in searchFashionImages',
        error: e,
  ...
  /// Search Pexels API
  static Future<List<OnlineInspiration>> _searchPexels(
    String query,
    int limit,
  ) async {
    final url =
        '$_pexelsBaseUrl/search?query=${Uri.encodeComponent('$query fashion outfit')}&per_page=$limit';
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        final inspirations = photos
            .map(
              (photo) => OnlineInspiration(
        ...
        AppLogger.debug(
          '✅ Pexels processing complete',
          data: {'processed': inspirations.length},
        );
        return inspirations;
      } else {
        AppLogger.warning(
          '❌ Pexels API returned error',
  ...
  /// Search Unsplash API
  static Future<List<OnlineInspiration>> _searchUnsplash(
    String query,
    int limit,
  ) async {
    final url =
        '$_unsplashBaseUrl/search/photos?query=${Uri.encodeComponent('$query fashion')}&per_page=$limit';
    AppLogger.network(url, 'GET');
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        AppLogger.debug(
          '✅ Unsplash processing complete',
          data: {'processed': inspirations.length},
        );
        return inspirations;
      } else {
        AppLogger.warning(
          '❌ Unsplash API returned error',
          error: response.statusCode,
        );
  ...
  static Future<List<OnlineInspiration>> searchClothingItems({
    ...
    AppLogger.info(
      '🎯 Clothing items search complete',
      data: {'results': result.length},
    );
    return result;
  }
  ...
  static Future<List<OnlineInspiration>> searchOutfitInspiration({
    ...
    final result = await searchFashionImages(query: query, limit: limit);
    AppLogger.info(
      '🎯 Outfit inspiration search complete',
      data: {'results': result.length},
    );
    return result;
  }
